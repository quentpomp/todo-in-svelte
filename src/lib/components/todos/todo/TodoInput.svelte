<script>
  ...
  /**
   * @type {{ id: string, complete: boolean, text: string}}
   */
  export let todo;
...
<div class="flex items-center justify-between rounded-md border-2 border-gray-one px-5 py-4">
  <div class="flex w-full max-w-lg items-center justify-start">
    ...
    <input
      ...
      value={todo.text}
      class="ml-5 flex-1 text-ellipsis rounded-none border-x-0 border-t-0 border-bottom border-dashed border-b-gray-two bg-cream-four px-0 pb-1 text-base font-normal text-gray-three placeholder:text-gray-two focus:border-gray-three focus:outline-none focus:ring-0"
    ...
    <button class="group ml-4 flex items-center justify-center rounded-md bg-cream-four p-2 hover:bg-steel-one focus:outline-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-five" type="button" on:click={() => deleteTodo(todo.id)}>
      <span class='sr-only'>
        Delete todo
        <svg
