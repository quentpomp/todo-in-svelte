<script>
    ...
    $: completeCount = $todos.filter((/** @type {{ id: string, complete: boolean, text: string}} */ todo) => todo.complete).length
    $: progressCount = $todos.filter((/** @type {{ id: string, complete: boolean, text: string}} */ todo) => !todo.complete).length
